...
def relay_control(relay_number,active):
   Relay_Status[relay_number] = active
   if (active):
      GPIO.output(Relays[relay_number],1)
   else:
      GPIO.output(Relays[relay_number],0)
...
while(True):
   ...
   for a in range (1,NUM_WATER_VALVES+1): # Pump water through all valves ones at a time
      APIpump("Valve " + str(a) + " ON", flow_count) # Before relay engage incase web api delay
      total_flow_count =  total_flow_count + flow_count
      flow_count = 0
      relay_control(a,True) # Turn valve on
      log_water_valve_status()
      time.sleep(PUMP_DELAY)

      relay_control(0,True) # Turn on water pump (Zero is pump relay)
      log_water_valve_status()
      time.sleep(WATER_CYCLE_LENGTH)

      relay_control(0,False) # Turn off water pump (Zero is pump relay)
      log_water_valve_status()
      time.sleep(PUMP_DELAY)

      relay_control(a,False) # Turn off valve
      log_water_valve_status()
   ...
   if (BALANCE_PH and (pH_time_limit - time.time()) < 1):
      ...
      if (current_pH < 2 or current_pH > 12): # Prevent some odd pH reading from impacting auto ajustment
         ...
         if (current_pH < lower_pH): # pH too low, make higher
            ...
            relay_control(PH_UP_RELAY,True)
            time.sleep(PH_VALVE_TIME)
            relay_control(PH_UP_RELAY,False)

         if (current_pH > upper_pH): # pH too high, make lower
            AGsys("Making pH lower")
            relay_control(PH_DOWN_RELAY,True)
            time.sleep(PH_VALVE_TIME)
            relay_control(PH_DOWN_RELAY,False)
